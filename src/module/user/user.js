...
const db = require("../../Database/models/Centroid")
const user_db = require("../../Database/models/User")
const Centroid = require("../hive/Centroid")
    ...
    return new Promise((resolve, rejects) => {
      ...
      crypto.generateKeyPair(
        ...
        async (err, publicKey, privateKey) => {
          ...
          if (!err) {
            // Hexing Password
            publicKey = Bufferfrom("publicKey")
            publicKey = publicKey.toString("hex")
            privateKey = Bufferfrom("privateKey")
            const key = privateKey.toString("hex")
            // Adding Salt
            const walletid = salt + "." + publicKey

            // Genarating Block
            const block_data = await centroid.push(
              walletid,
              `{"Login_ID":${username}}`,
              "0"
            )
