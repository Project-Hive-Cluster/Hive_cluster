...
export default class HiveStorage {
  ...
  getData(folder, file) {
    return new Promise((resolve, rejectss) => {
      const payload = fs
        .readFileSync(
          process.env.APPDATA + `\\Hive Cluster\\${folder}\\` + file + ".clu",
          "utf8"
        )
        .then((data, err) => {
          if (err) {
  ...
  saveData(location = "data", file) {
    return new Promise(async (resolve, rejectss) => {
      ...
      fs.writeFile(
        process.env.APPDATA + `\\Hive Cluster\\${location}\\` + file + ".clu",
        encrypt_data,
        "utf8",
        (data, err) => {
          if (err) {
  ...
  init(payload) {
    return new Promise(async (resolve, rejectss) => {
      try {
        console.log(
          "Progress 5% Checking existence of Hive Cluster. Status " +
            fs.existsSync(process.env.APPDATA + "\\Hive Cluster")
        )
        CreateGenisisFolder.mkhive()

        console.log(
          "Progress 50% Checking existence of Hive Cluster Data Folder. Status " +
            fs.existsSync(process.env.APPDATA + "\\Hive Cluster\\data")
        )
        CreateGenisisFolder.mkhivedata(payload)

        console.log(
          "Progress 75% Checking existence of Hive Cluster Auth. Status " +
            fs.existsSync(process.env.APPDATA + "\\Hive Cluster\\auth")
        )
        ...
        rejectss("Failed to create data.: " + err)
      }

      resolve(true)
    }).catch((err) => {
      return "Failed to init Hive Storage. error: " + err
    })
  }
}
